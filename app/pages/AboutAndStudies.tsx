...
export default function AboutAndStudies(): JSX.Element {
  ...
  useLayoutEffect(() => {
    const aboutEl = aboutRef.current; // copy ref
    if (!aboutEl) return;

    const measure = () => {
      const h = Math.ceil(
        aboutEl.offsetHeight || aboutEl.getBoundingClientRect().height
      );
      setAboutHeight(h);
    };
    ...
    >
      <div className="mx-auto w-full grid grid-cols-1 md:grid-cols-[2fr_1fr] gap-6 md:items-stretch items-start h-full min-h-0">
        ...
        <div
          ref={rightCardRef}
          style={
            aboutHeight && isDesktop
              ? { minHeight: `${aboutHeight}px` }
              : undefined
          }
